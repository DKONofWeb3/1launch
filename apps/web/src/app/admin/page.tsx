  ...
  return (
    <div style={{ background: '#0E0E16', border: '1px solid #1E1E2E', borderRadius: 12, padding: '18px 20px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 3, height: 80 }}>
        {data.map((d, i) => (
          <div key={i} title={`${d.date}: $${d.revenue}`} style={{ flex: 1 }}>
            <div style={{
              width: '100%',
    ...
    <div style={{ minHeight: '100vh', background: '#0A0A0F', padding: '24px' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          ...
          <button onClick={() => loadTab(tab)} style={{ padding: '8px 16px', background: 'transparent', border: '1px solid #1E1E2E', borderRadius: 6, fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, color: '#6B7280', cursor: 'pointer' }}>
            Refresh
          </button>
        </div>
        ...
        {error && (
          <div style={{ padding: '10px 14px', background: 'rgba(255,59,59,0.08)', border: '1px solid rgba(255,59,59,0.2)', borderRadius: 8, fontFamily: 'IBM Plex Mono, monospace', fontSize: 12, color: '#FF6B6B', marginBottom: 16 }}>
            {error}
          </div>
        )}
        ...
        {tab === 'overview' && !loading && overview && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12 }}>
              ...
              <StatCard label="Tokens This Month"  value={overview.tokens_month || 0}          sub="last 30 days"       />
              <StatCard label="BSC Launches"       value={overview.bsc_tokens || 0}            sub={`$${(overview.bsc_tokens || 0) * 15} collected`} color="#F3BA2F" />
              <StatCard label="SOL Launches"       value={overview.sol_tokens || 0}            sub={`$${(overview.sol_tokens || 0) * 6} collected`}  color="#9945FF" />
              <StatCard label="Total Users"        value={overview.total_users || 0}           sub="connected wallets"  />
              <StatCard label="TG Linked Users"    value={overview.tg_linked_users || 0}       sub="wallet + TG synced" color="#27AEE6" />
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div style={{ background: '#0E0E16', border: '1px solid #1E1E2E', borderRadius: 12, padding: '18px 20px' }}>
                ...
                {[
                  { label: 'BSC Deploy Fees ($15)',  value: fmt((overview.bsc_tokens || 0) * 15),  color: '#F3BA2F' },
                  { label: 'SOL Deploy Fees ($6)',   value: fmt((overview.sol_tokens || 0) * 6),   color: '#9945FF' },
                  { label: 'Volume Bot',             value: fmt(overview.volbot_revenue || 0),      color: '#00FF88' },
                ].map(r => (
              ...
              <div style={{ background: '#0E0E16', border: '1px solid #1E1E2E', borderRadius: 12, padding: '18px 20px' }}>
                <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 10, color: '#4B5563', letterSpacing: '0.1em', textTransform: 'uppercase' as const, marginBottom: 14 }}>Recent Activity</div>
                {(overview.recent_tokens || []).slice(0, 5).map((t: any, i: number) => (
                ...
                {!(overview.recent_tokens?.length) && (
                  <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 12, color: '#374151' }}>No tokens yet</div>
                )}
              </div>
            </div>
        ...
        {tab === 'users' && !loading && (
          <div style={{ background: '#0E0E16', border: '1px solid #1E1E2E', borderRadius: 12, overflow: 'hidden' }}>
            ...
            {/* Header */}
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr 1fr 1fr 1fr 1fr', gap: 8, padding: '10px 20px', background: '#0A0A0F', borderBottom: '1px solid #1E1E2E' }}>
              {['Wallet', 'Telegram', 'Tokens', 'TG Linked', 'Revenue', 'Joined'].map(h => (
            ...
            {users.map((u, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr 1fr 1fr 1fr 1fr', gap: 8, padding: '12px 20px', borderBottom: '1px solid #0A0A0F', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)', alignItems: 'center' }}>
                <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, color: '#9CA3AF' }}>{shortAddr(u.wallet_address)}</span>
                <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, color: u.telegram_username ? '#27AEE6' : '#374151' }}>
                ...
                <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 10 }}>
                  {u.telegram_id ? <Badge label="yes" color="#00FF88" /> : <Badge label="no" color="#374151" />}
                </span>
            ...
            {users.length === 0 && (
              <div style={{ padding: '40px', textAlign: 'center', fontFamily: 'IBM Plex Mono, monospace', fontSize: 12, color: '#374151' }}>No users yet</div>
            )}
          </div>
        )}
        ...
        {tab === 'payments' && !loading && (
          <div style={{ background: '#0E0E16', border: '1px solid #1E1E2E', borderRadius: 12, overflow: 'hidden' }}>
            <div style={{ padding: '14px 20px', borderBottom: '1px solid #1E1E2E', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, color: '#00FF88', fontWeight: 700 }}>
                Total confirmed: {fmt(payments.filter(p => p.status === 'confirmed').reduce((sum: number, p: any) => sum + (p.usd_amount || 0), 0))}
              </span>
            </div>
            ...
            {payments.map((p, i) => {
              const typeLabel = p.plan_id === 'deploy_fee_bsc' ? 'BSC Deploy'
                : p.plan_id === 'deploy_fee_sol' ? 'SOL Deploy'
                : p.plan_id?.startsWith('volbot_') ? `Vol Bot (${p.plan_id.replace('volbot_', '')})`
                : p.plan_id || 'Unknown'
              const typeColor = p.plan_id?.includes('bsc') ? '#F3BA2F'
                : p.plan_id?.includes('sol') ? '#9945FF'
                : p.plan_id?.includes('volbot') ? '#00FF88'
                : '#6B7280'
              return (
            ...
            {payments.length === 0 && (
              <div style={{ padding: '40px', textAlign: 'center', fontFamily: 'IBM Plex Mono, monospace', fontSize: 12, color: '#374151' }}>No payments yet</div>
            )}
          </div>
        )}
        ...
        {tab === 'tokens' && !loading && (
          <div style={{ background: '#0E0E16', border: '1px solid #1E1E2E', borderRadius: 12, overflow: 'hidden' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 0.8fr 0.8fr 2fr 1fr 1fr 1fr', gap: 8, padding: '10px 20px', background: '#0A0A0F', borderBottom: '1px solid #1E1E2E' }}>
              {['Name', 'Ticker', 'Chain', 'Contract', 'Wallet', 'MCap', 'Launched'].map(h => (
                <span key={h} style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 9, color: '#4B5563', letterSpacing: '0.1em', textTransform: 'uppercase' as const }}>{h}</span>
              ))}
            </div>
            {tokens.map((t, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.5fr 0.8fr 0.8fr 2fr 1fr 1fr 1fr', gap: 8, padding: '12px 20px', borderBottom: '1px solid #0A0A0F', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)', alignItems: 'center' }}>
                ...
                <Badge label={t.chain} color={t.chain === 'bsc' ? '#F3BA2F' : '#9945FF'} />
                <a href={t.chain === 'bsc' ? `https://bscscan.com/token/${t.contract_address}` : `https://solscan.io/token/${t.contract_address}`}
                ...
                <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, color: t.market_cap_usd > 0 ? '#F9FAFB' : '#374151' }}>
                  {t.market_cap_usd > 0 ? fmt(t.market_cap_usd) : '—'}
                </span>
            ...
            {tokens.length === 0 && (
              <div style={{ padding: '40px', textAlign: 'center', fontFamily: 'IBM Plex Mono, monospace', fontSize: 12, color: '#374151' }}>No tokens yet</div>
            )}
          </div>
        )}
        ...
        {tab === 'broadcast' && (
          <div style={{ maxWidth: 600, display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ background: '#0E0E16', border: '1px solid #1E1E2E', borderRadius: 12, padding: '20px 24px' }}>
              ...
              <button onClick={handleBroadcast} disabled={broadcasting || !broadcastMsg}
                style={{ width: '100%', padding: '10px', background: broadcasting || !broadcastMsg ? '#1E1E2E' : '#00FF88', border: 'none', borderRadius: 6, fontFamily: 'IBM Plex Mono, monospace', fontSize: 12, fontWeight: 700, color: broadcasting || !broadcastMsg ? '#374151' : '#0A0A0F', cursor: broadcasting ? 'not-allowed' : 'pointer' }}>

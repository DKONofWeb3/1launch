...
function useBSCDeploy() {
  ...
  async function deploy(draft: any, factoryAddress: string): Promise<DeployResult> {
    ...
    try {
      const parsed = parseEventLogs({ abi: FACTORY_ABI, eventName: 'TokenCreated', logs: receipt.logs })
      if (parsed.length > 0) tokenAddress = (parsed[0].args as any).token as string
    } catch {}

    if (!tokenAddress) {
      const factoryLog = receipt.logs.find((log: any) => log.address?.toLowerCase() === factoryAddress.toLowerCase())
      if (factoryLog && factoryLog.topics[1]) tokenAddress = '0x' + factoryLog.topics[1].slice(26)
    }
      ...
      for (const log of receipt.logs) {
        for (const topic of log.topics.slice(1)) {
          if (topic && topic.length === 66) {
            ...
            if (candidate.toLowerCase() !== factoryAddress.toLowerCase() && candidate.toLowerCase() !== address!.toLowerCase()) {
...
function BoostModal({ contractAddress, chain, draftId, onClose }: {
  contractAddress: string; chain: string; draftId: string; onClose: () => void
}) {
  ...
  async function payWithWallet() {
    ...
    try {
      ...
      const bnbPrice  = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=binancecoin&vs_currencies=usd').then(r => r.json()).then(d => d.binancecoin.usd).catch(() => 603)
      ...
      const initRes = await api.post('/api/subscriptions/initiate', { plan_id: `volbot_${tier}`, chain: 'bsc', token: 'BNB', wallet: address, meta: { contract_address: contractAddress, draft_id: draftId } })
      ...
      await api.post('/api/payments/verify-tx', { tx_hash: txHash, chain: 'bsc', payment_id: initRes.data.data.id })
      setDone(true)
    } catch (err: any) {
  ...
  const overlay: React.CSSProperties = { position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(10,10,15,0.94)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 20px' }
  const card: React.CSSProperties   = { background: '#0E0E16', border: '1px solid #1E1E2E', borderRadius: 16, padding: '24px', maxWidth: 440, width: '100%' }

  if (done) return (
    <div style={overlay}><div style={{ ...card, textAlign: 'center' }}>
  ...
  return (
    <div style={overlay}><div style={card}>
      ...
      <p style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, color: '#6B7280', lineHeight: 1.7, marginBottom: 16 }}>Simulates real trading activity so your token doesn't look dead. First 5 minutes matter most.</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
        {tiers.map(t => (
          <button key={t.id} onClick={() => setTier(t.id)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 14px', background: tier === t.id ? 'rgba(0,255,136,0.06)' : '#0A0A0F', border: `1.5px solid ${tier === t.id ? 'rgba(0,255,136,0.3)' : '#1E1E2E'}`, borderRadius: 8, cursor: 'pointer', width: '100%', textAlign: 'left' }}>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={payWithWallet} disabled={!tier || loading} style={{ flex: 2, padding: '12px 0', background: tier && !loading ? '#00FF88' : '#1E1E2E', border: 'none', borderRadius: 8, cursor: tier && !loading ? 'pointer' : 'not-allowed', fontFamily: 'IBM Plex Mono, monospace', fontSize: 12, fontWeight: 700, color: tier && !loading ? '#0A0A0F' : '#374151' }}>{loading ? 'Confirm in MetaMask...' : 'Boost My Token'}</button>
...
function WhitepaperAddon({ contractAddress, draftId }: { contractAddress: string; draftId: string }) {
  ...
  async function payAndGenerate() {
    ...
    try {
      ...
      const platformWallet = process.env.NEXT_PUBLIC_PLATFORM_WALLET_ADDRESS as `0x${string}`
      if (!platformWallet) throw new Error('Platform wallet not configured')
      const initRes = await api.post('/api/subscriptions/initiate', { plan_id: 'whitepaper', chain: 'bsc', token: 'BNB', wallet: address })
      if (!initRes.data.success) throw new Error(initRes.data.error)
      const txHash = await sendBNB({ to: platformWallet, value: parseEther(bnbAmount) })
      if (!txHash) throw new Error('Transaction cancelled')
      await api.post('/api/payments/verify-tx', { tx_hash: txHash, chain: 'bsc', payment_id: initRes.data.data.id })
      ...
      setError(err.message || 'Failed'); setStep('idle')
    }
  }
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', background: '#0A0A0F', border: '1px solid #1E1E2E', borderRadius: 8 }}>
      ...
      <button onClick={payAndGenerate} disabled={step !== 'idle'} style={{ padding: '6px 14px', background: step === 'idle' ? 'rgba(255,149,0,0.08)' : 'transparent', border: `1px solid ${step === 'idle' ? 'rgba(255,149,0,0.3)' : '#1E1E2E'}`, borderRadius: 6, cursor: step === 'idle' ? 'pointer' : 'not-allowed', fontFamily: 'IBM Plex Mono, monospace', fontSize: 10, fontWeight: 700, color: step === 'idle' ? '#FF9500' : '#4B5563', flexShrink: 0 }}>
        {step === 'idle' ? '$15' : '...'}
      </button>
    </div>
  )
...
function PostDeployModal({ result, draft, chain, draftId, onDismiss }: {
  ...
  return (
    <>
      {showBoost && <BoostModal contractAddress={result.contractAddress} chain={chain} draftId={draftId} onClose={() => setShowBoost(false)} />}
      <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(10,10,15,0.96)', backdropFilter: 'blur(12px)', overflowY: 'auto', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '40px 20px 60px' }}>
        <div style={{ maxWidth: 520, width: '100%', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: '#0E0E16', border: '1px solid #1E1E2E', borderRadius: 12, padding: '16px 18px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
              <div style={{ flex: 1, fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, color: '#F9FAFB', wordBreak: 'break-all' }}>{result.contractAddress}</div>
              <button onClick={() => { navigator.clipboard.writeText(result.contractAddress); setCopied(true); setTimeout(() => setCopied(false), 2000) }} style={{ padding: '6px 12px', flexShrink: 0, background: copied ? 'rgba(0,255,136,0.1)' : 'transparent', border: `1px solid ${copied ? 'rgba(0,255,136,0.3)' : '#1E1E2E'}`, borderRadius: 6, cursor: 'pointer', fontFamily: 'IBM Plex Mono, monospace', fontSize: 10, color: copied ? '#00FF88' : '#6B7280' }}>
                {copied ? 'Copied' : 'Copy'}
              </button>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <a href={result.explorerUrl} target="_blank" rel="noreferrer" style={{ flex: 1, padding: '8px 0', textAlign: 'center', background: '#00FF88', border: 'none', borderRadius: 7, fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, fontWeight: 700, color: '#0A0A0F', textDecoration: 'none' }}>View on Explorer</a>
              <a href={result.txUrl} target="_blank" rel="noreferrer" style={{ flex: 1, padding: '8px 0', textAlign: 'center', background: 'transparent', border: '1px solid #1E1E2E', borderRadius: 7, fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, color: '#6B7280', textDecoration: 'none' }}>View Tx</a>
            </div>
          </div>
          <div style={{ background: '#0E0E16', border: '1px solid #1E1E2E', borderRadius: 12, padding: '16px 18px' }}>
            <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 9, color: '#4B5563', letterSpacing: '0.12em', marginBottom: 14 }}>NEXT STEPS</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <a href={chain === 'bsc' ? `https://pancakeswap.finance/add/${result.contractAddress}` : `https://raydium.io/liquidity/create-pool/`} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', background: '#0A0A0F', border: '1px solid #1E1E2E', borderRadius: 8, textDecoration: 'none' }}>
                <div>
              ...
              <a href={chain === 'bsc' ? `https://app.unicrypt.network/amm/pancake-v2/pair` : `https://app.unicrypt.network/amm/raydium/pair`} target="_blank" rel="noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', background: '#0A0A0F', border: '1px solid #1E1E2E', borderRadius: 8, textDecoration: 'none' }}>
                <div>
              ...
              <button onClick={() => { navigator.clipboard.writeText(launchTweet); setTweetCopied(true); setTimeout(() => setTweetCopied(false), 2000) }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', background: tweetCopied ? 'rgba(0,255,136,0.04)' : '#0A0A0F', border: `1px solid ${tweetCopied ? 'rgba(0,255,136,0.2)' : '#1E1E2E'}`, borderRadius: 8, cursor: 'pointer', width: '100%', textAlign: 'left' }}>
                <div>
          ...
          <div style={{ background: '#0E0E16', border: '1px solid #2A2A3E', borderRadius: 12, padding: '18px' }}>
            ...
            <p style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, color: '#6B7280', lineHeight: 1.7, marginBottom: 14 }}>Volume Bot simulates real trading activity so your token doesn't look dead at launch.</p>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setShowBoost(true)} style={{ flex: 2, padding: '12px 0', background: '#00FF88', border: 'none', borderRadius: 8, cursor: 'pointer', fontFamily: 'IBM Plex Mono, monospace', fontSize: 12, fontWeight: 700, color: '#0A0A0F' }}>Boost My Token</button>
              <button onClick={onDismiss} style={{ flex: 1, padding: '12px 0', background: 'transparent', border: '1px solid #1E1E2E', borderRadius: 8, cursor: 'pointer', fontFamily: 'IBM Plex Mono, monospace', fontSize: 12, color: '#4B5563' }}>Skip for now</button>
            </div>
          </div>
        </div>
      </div>
...
function DeployPageContent() {
  ...
  // BSC wallet is the user identity. Always use evmAddress for user_id lookup.
  const activeEvmWallet = evmAddress || bscAddress || ''
  ...
  async function handleDeploy() {
    if (isTelegram && !activeEvmWallet && !activeSolAddress) { setTgWalletGate(true); return }
    setError(null)
      ...
      if (!isSolana) {
        ...
        if (!config?.factoryAddress || !bscAddress) {
          ...
          const res = await api.post('/api/deploy/server-side', {
            ...
            chain:         'bsc',
            network:       'mainnet',
            wallet_address: activeEvmWallet,  // BSC wallet = user identity
          })
        ...
        setStatus('waiting_wallet')
        const deployResult = await deployBSC(draft, config.factoryAddress)
        ...
        const res = await api.post('/api/deploy/server-side', {
          ...
          chain:                'solana',
          network:              'mainnet',
          wallet_address:       activeEvmWallet,   // ← BSC wallet for user identity (NEVER solana address)
          solana_owner_address: activeSolAddress || '',  // ← where Solana tokens get sent
        })
        if (!res.data.success) throw new Error(res.data.error)
        setStatus('success')
        setResult({
  ...
  async function payWithSolanaWallet() {
    setPayingNow(true); setPayError(null)
    try {
      const solPrice  = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=solana&vs_currencies=usd').then(r => r.json()).then(d => d.solana.usd).catch(() => 81)
      ...
      const initRes = await api.post('/api/subscriptions/initiate', { plan_id: 'deploy_fee', chain: 'solana', token: 'SOL', wallet: activeSolAddress || 'tg_user' })
      if (!initRes.data.success) throw new Error(initRes.data.error)
      const signature = await paySOL(solAmount, toAddress)
      await api.post('/api/payments/verify-tx', { tx_hash: signature, chain: 'solana', payment_id: initRes.data.data.id })
      setPaymentPaid(true)
    } catch (err: any) {
  ...
  if (tgWalletGate) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '70vh', padding: '24px 20px', textAlign: 'center' }}>
      ...
      <p style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 12, color: '#6B7280', marginBottom: 28, maxWidth: 300, lineHeight: 1.7 }}>Connect to receive 100% of the token supply to your wallet.</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, width: '100%', maxWidth: 340 }}>
        <button onClick={() => openConnectModal?.()} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', background: '#0E0E16', border: '1px solid rgba(243,186,47,0.3)', borderRadius: 10, cursor: 'pointer', textAlign: 'left' as const, width: '100%' }}>
          <div style={{ width: 40, height: 40, borderRadius: 8, background: 'rgba(243,186,47,0.1)', border: '1px solid rgba(243,186,47,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M12 2L14.5 7H9.5L12 2Z" fill="#F3BA2F"/><path d="M7 5.5L9.5 7L7 9.5L4.5 7L7 5.5Z" fill="#F3BA2F"/><path d="M17 5.5L19.5 7L17 9.5L14.5 7L17 5.5Z" fill="#F3BA2F"/><path d="M12 7L17 9.5L19.5 12L17 14.5L12 17L7 14.5L4.5 12L7 9.5L12 7Z" fill="#F3BA2F"/><path d="M7 14.5L9.5 17L7 19.5L4.5 17L7 14.5Z" fill="#F3BA2F"/><path d="M17 14.5L19.5 17L17 19.5L14.5 17L17 14.5Z" fill="#F3BA2F"/><path d="M12 17L14.5 19L12 22L9.5 19L12 17Z" fill="#F3BA2F"/></svg>
          </div>
          <div>
        ...
        <button onClick={() => openSolModal(true)} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', background: '#0E0E16', border: '1px solid rgba(153,69,255,0.3)', borderRadius: 10, cursor: 'pointer', textAlign: 'left' as const, width: '100%' }}>
          <div style={{ width: 40, height: 40, borderRadius: 8, background: 'rgba(153,69,255,0.1)', border: '1px solid rgba(153,69,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M4 17.5H20L17 20H4L4 17.5Z" fill="url(#sol1)"/><path d="M4 10.75H20L17 13.25H4L4 10.75Z" fill="url(#sol2)"/><path d="M4 4H20L17 6.5H4L4 4Z" fill="url(#sol3)"/><defs><linearGradient id="sol1" x1="4" y1="18.75" x2="20" y2="18.75" gradientUnits="userSpaceOnUse"><stop stopColor="#9945FF"/><stop offset="1" stopColor="#14F195"/></linearGradient><linearGradient id="sol2" x1="4" y1="12" x2="20" y2="12" gradientUnits="userSpaceOnUse"><stop stopColor="#9945FF"/><stop offset="1" stopColor="#14F195"/></linearGradient><linearGradient id="sol3" x1="4" y1="5.25" x2="20" y2="5.25" gradientUnits="userSpaceOnUse"><stop stopColor="#9945FF"/><stop offset="1" stopColor="#14F195"/></linearGradient></defs></svg>
          </div>
          <div>
      ...
      <button onClick={() => setTgWalletGate(false)} style={{ marginTop: 20, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, color: '#374151' }}>Cancel</button>
    </div>
  )
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {!paymentPaid && isSolana && (
            <div style={{ background: '#0E0E16', border: '1px solid #1E1E2E', borderRadius: 12, padding: '18px 20px' }}>
              ...
              {payError && <div style={{ padding: '8px 12px', background: 'rgba(255,59,59,0.08)', border: '1px solid rgba(255,59,59,0.2)', borderRadius: 6, fontFamily: 'IBM Plex Mono, monospace', fontSize: 11, color: '#FF6B6B', marginBottom: 10 }}>{payError}</div>}
              {!activeSolAddress ? (
                ...
                <button onClick={payWithSolanaWallet} disabled={payingNow} style={{ width: '100%', padding: '13px', background: payingNow ? '#1E1E2E' : '#9945FF', color: '#F9FAFB', border: 'none', borderRadius: 8, fontFamily: 'IBM Plex Mono, monospace', fontSize: 13, fontWeight: 700, cursor: payingNow ? 'not-allowed' : 'pointer' }}>
                  {payingNow ? 'Confirm in wallet...' : 'Pay $1 in SOL'}
                </button>
              )}
              <p style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 10, color: '#374151', marginTop: 8 }}>Your Solana wallet will open to confirm the payment.</p>
            </div>
          )}
          ...
          <div style={{ opacity: paymentPaid ? 1 : 0.35, pointerEvents: paymentPaid ? 'auto' : 'none', transition: 'opacity 0.3s' }}>
            ...
            <button onClick={handleDeploy} disabled={!paymentPaid} style={{ width: '100%', padding: '14px', background: paymentPaid ? '#00FF88' : '#1E1E2E', color: paymentPaid ? '#0A0A0F' : '#374151', border: 'none', borderRadius: 10, fontFamily: 'IBM Plex Mono, monospace', fontSize: 14, fontWeight: 700, cursor: paymentPaid ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.15s' }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0A0F' }}>
      <div style={{ borderBottom: '1px solid #1E1E2E', padding: '0 24px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(10,10,15,0.9)', position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <div style={{ width: 120, display: 'flex', justifyContent: 'flex-end' }}><MultiWalletButton /></div>
      </div>
